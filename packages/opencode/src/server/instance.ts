...
export const InstanceRoutes = (upgrade: UpgradeWebSocket, app: Hono = new Hono()) =>
  app
    ...
    .post("/insights", async (c) => {
      ...
      const resolvedModel = smallModel ?? (await Provider.getModel(defaultModel.providerID, defaultModel.modelID))
      if (!resolvedModel) return c.json({ error: "No model available for insights analysis" }, 400)
      const languageModel = await Provider.getLanguage(resolvedModel)
